...
const NavBar = () => {
  ...
  const renderButton = (screenName, label, icon) => {
    ...
    const buttonStyle =
      IsFocused && navigation.getState().routes[navigation.getState().index].name === screenName
        ? styles.activeButton
        : styles.button;
    ...
    return (
      <TouchableOpacity style={buttonStyle} onPress={() => navigation.navigate(screenName)}>
  ...
  return (
    <View style={[styles.NavBarContainer, { paddingHorizontal: screenWidth * 0.05 }]}>
